...
function CollapsibleSection({
  ...
  return (
    <div>
      ...
      >
        <svg
          className={`w-3 h-3 ${colorClass} transition-transform duration-200 ${open ? 'rotate-90' : ''}`}
          fill="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <span className={`text-xs font-bold ${colorClass} opacity-70`}>({count})</span>
      </button>
      <div className={`section-collapse ${open ? 'section-collapse-open' : ''}`}>
...
const DiyStepCard = memo(function DiyStepCard({ step, analysis, forceExpanded, index = 0 }: DiyStepCardProps) {
  ...
  return (
    <div
      className={`diy-step-card glass-card ${riskClass}`}
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-sm font-semibold leading-tight">{step.action_summary}</h3>
            {analysis && (
        ...
        {/* Expand chevron */}
        <svg
          className={`w-4 h-4 text-muted shrink-0 transition-transform duration-300 ${expanded ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <div className={`step-card-collapse ${expanded ? 'step-card-collapse-open' : ''}`}>
        <div className="step-card-collapse-inner">
          <div className="px-5 pb-5 pt-3 border-t border-card-border space-y-4">
            ...
            {analysis ? (
              <div className="space-y-3 pt-1">
                ...
                <CollapsibleSection title="Required Precautions" count={analysis.required_precautions.length} colorClass="text-blue-400">
                  <PrecautionList items={analysis.required_precautions} colorClass="text-blue-400" icon="warning" />
                </CollapsibleSection>

                {/* Already mentioned in video */}
                <CollapsibleSection title="Already Mentioned" count={analysis.already_mentioned_precautions.length} colorClass="text-emerald-400" defaultOpen={false}>
                  <PrecautionList items={analysis.already_mentioned_precautions} colorClass="text-emerald-400" icon="check" />
                </CollapsibleSection>

                {/* Missing precautions */}
                <CollapsibleSection title="Missing Precautions" count={analysis.missing_precautions.length} colorClass="text-orange-400">
                  <PrecautionList items={analysis.missing_precautions} colorClass="text-orange-400" icon="x" />
                </CollapsibleSection>

                {/* Matched rules */}
                <CollapsibleSection title="Matched Safety Rules" count={analysis.matched_rules.length} colorClass="text-yellow-400" defaultOpen={false}>
